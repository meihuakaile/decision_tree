...
def plot_text(parent_pt, center_parent, text):
    ...
    # horizontalalignment = ha  ['center' | 'right' | 'left'] 水平对齐
    # verticalalignment or va['center' | 'top' | 'bottom' | 'baseline']  垂直对齐
    ax.text(x, y, text, va='center', ha='center', rotation=30)
...
tree = {'tearRate': {'reduced': 'no lenses', 'normal': {'astigmatic': {'yes': {'prescript': {'hyper': {'age': {'pre': 'no lenses', 'presbyopic': 'no lenses', 'young': 'hard'}}, 'myope': 'hard'}}, 'no': {'age': {'pre': 'soft', 'presbyopic': {'prescript': {'hyper': 'soft', 'myope': 'no lenses'}}, 'young': 'soft'}}}}}}
tree = load_tree()
